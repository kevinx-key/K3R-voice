...
static int Run(LPTSTR lpCmdLine) {
  ...
  if (!wcscmp(L"/?", lpCmdLine) || !wcscmp(L"/help", lpCmdLine)) {
    ...
    if (LoadString(GetModuleHandle(NULL), IDS_STR_HELP, msg,
      ...
      MessageBox(NULL, msg, L"【倾听输入法】", MB_ICONINFORMATION | MB_OK);
    } else {
      MessageBox(
          NULL,
          L"Usage: WeaselSetup.exe [options]\n"
          L"/? or /help    - Show this help message\n"
          L"/u             - Uninstall Listen\n"
          L"/i             - Install Listen\n"
          L"/s             - Install Listen (Simplified Chinese)\n"
          L"/t             - Install Listen (Traditional Chinese)\n"
          L"/ls            - Set Listen language to Simplified Chinese\n"
          L"/lt            - Set Listen language to Traditional Chinese\n"
          L"/le            - Set Listen language to English\n"
          ...
          L"【倾听输入法】", MB_ICONINFORMATION | MB_OK);
    }
    return 0;
  }
